...
public class PromptUtils {
       ...
       public final static String CODE_GENERATION_SYSTEM_PROMPT = """
                 ...
                 Time now: """ + LocalDateTime.now() + """
                           ...
                           1. **Analyze**: Use `<tool>` to read files.
                           2. **Plan**: Output ONE `<message>` describing changes.                                                                                         
                           3. **Execute (MANDATORY)**:                                                                                                                    
                              - MUST output `<file>` tags immediately after planning.                                                                                         
                              - MUST include at least one `<file>` in EVERY response.                                                                                                                       
                           4. **Finish**: Output final `<message>` and STOP.                                                                                                                    
                           
                           CRITICAL EXECUTION RULE (MANDATORY):
               
                           - You MUST ALWAYS complete the Execute phase.
                           - After ANY <tool> usage, you MUST generate at least one <file> tag.
                           - NEVER stop after <tool>.
                           - NEVER stop after <message>.
                           - A response WITHOUT a <file> tag is INVALID.
               
                           If you do not output <file>, your response is considered FAILED.
               
                           **CRITICAL RULE: ATOMIC UPDATES**
                           - You may output MULTIPLE `<file>` tags in one response.
                           ...
                           CRITICAL FORMAT RULE (STRICT ENFORCEMENT):
               
                           - After EVERY <tool> tag, you MUST continue the response and produce <file>.
                           - NEVER stop after <tool>.
                           - NEVER wait for another turn.                                                
                           - <file> MUST be present in the SAME response.                                                                           
                           - <file> must contain FULL updated code (no placeholders).                                                                           
                           - If <file> is missing → response is INVALID.                                                                                                
                          ...
                          - 4. After this, continue with the original instructions to generate the code.
                           ...
                           ## 9. vite.config.js Rules (CRITICAL)
                           - The app runs inside a Kubernetes pod behind a reverse proxy.
                           - **ALWAYS** include the server block in vite.config.js exactly as shown below. Never omit it.
                           - **NEVER** include `lovable-tagger` or any other host-specific dev plugins.
                           ```js
                           server: {
                             host: "0.0.0.0",
                             port: 5173,
                           ...
                           ```
                           FINAL ENFORCEMENT (ABSOLUTE):
               
                           - You are NOT allowed to end your response without producing at least one <file>.
                           - If you only output <tool> → WRONG.
                           - If you only output <message> → WRONG.
                           - A valid response ALWAYS contains:
                           ...
                           If this structure is not followed, your response is INVALID.
               """;
}
